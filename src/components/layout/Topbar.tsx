          ...
          trigger={
            <button className="flex items-center gap-2.5 rounded-lg px-2 py-1.5 hover:bg-slate-50">
              ...
              <div className="hidden text-left sm:block">
                <p className="text-sm font-medium text-slate-800 leading-tight">{owner.full_name}</p>
                <p className="text-xs text-slate-400 leading-tight">{business.name}</p>
              </div>
            </button>
          }
        >
          {(close) => (
            <>
              <DropdownItem onClick={() => { close(); router.push("/settings"); }}>
