...
export function PaymentStatusCell({
  ...
  return (
    <div className="flex flex-col gap-0.5">
      <div className={`relative h-9 w-24 rounded-lg border ${statusClasses[status]}`}>
        <select
          value={status}
          onChange={(e) => handleChange(e.target.value as PaymentStatus)}
          disabled={loading}
          className="h-full w-full cursor-pointer appearance-none bg-transparent pl-2.5 pr-5 text-sm font-medium focus:outline-none disabled:cursor-not-allowed"
        >
